...
{% if not cart_page %}
  ...
  <h5 class="js-visible-on-cart-filled row mb-1 font-big" {% if cart.items_count == 0 %}style="display:none;"{% endif %} data-store="cart-subtotal">
...
{% if cart_page %}
  ...
  <div id="cart-sticky-summary" class="position-sticky-md cart-page-totals mt-4 mt-md-0">
    {% if has_free_shipping_bar %}
      {# includes free shipping progress bar: only if store has free shipping with a minimum #}
    ...
    <h5 class="js-visible-on-cart-filled row font-weight-normal mb-0 font-big" {% if cart.items_count == 0 %}style="display:none;"{% endif %} data-store="cart-subtotal">
      ...
      {% for promotion in cart.promotional_discount.promotions_applied %}
        ...
        {% endif %}
          <span class="js-total-promotions-detail-row row" id="{{ id }}">
            ...
            <span class="col text-right pr-md-0">-{{ promotion.total_discount_amount_short }}</span>
          </span>
      {% endfor %}
    </div>
    ...
    {% if show_calculator_on_cart %}
      <div id="shipping-cost-container" class="js-fulfillment-info js-visible-on-cart-filled js-shipping-cost-table font-weight-normal row mt-2 mb-0" {% if cart.items_count == 0 or (not cart.has_shippable_products) %}style="display:none;"{% endif %}>
...
{% endif %}
    ...
    <div class="js-visible-on-cart-filled {% if cart_page %}row px-3 px-md-0{% endif %}" {% if cart.items_count == 0 %}style="display:none;"{% endif %}>
      ...
      {% if cart_page %}
        ...
        {% if cart.checkout_enabled %}
          <input id="go-to-checkout" class="btn btn-primary btn-big btn-block mb-3 mx-2 mx-md-0" type="submit" name="go_to_checkout" value="{{ 'Iniciar Compra' | translate }}"/>
        {% else %}

          {# Cart minium alert #}
          
          <div class="alert alert-warning mt-4 mb-3 col">
      ...
      {% else %}
        ...
        <div class="js-ajax-cart-minimum alert alert-warning mt-4 mb-3" {{ cart.checkout_enabled ? 'style="display:none"' }} id="ajax-cart-minumum-div">
          {{ "El monto mínimo de compra es de {1} sin incluir el costo de envío" | t(cart_total | money) }}
        </div>
