<div class="js-product-variants {% if quickshop %}js-product-quickshop-variants{% endif %} form-row pb-3">
	...
	{% for variation in product.variations %}
		...
		<div class="js-product-variants-group {% if variation.name in ['Color', 'Cor'] %}js-color-variants-container{% endif %} {% if settings.bullet_variants %}col-12 mb-2{% else %}{% if loop.length == 3 %}col-12 col-md-4{% elseif loop.length == 2 %}col-6{% else %}col-12 col-md-6{% endif %}{% endif %}" data-variation-id="{{ variation.id }}">
			{% if quickshop %}
	...
	{% if show_size_guide and settings.size_guide_url and has_size_variations %}
		...
		{% for page in pages if page.handle == size_guide_url_handle and not has_size_guide_page_finded %}
			...
			{% if has_size_guide_page_finded %}
				<a data-toggle="#size-guide-modal" data-modal-url="modal-fullscreen-size-guide" class="js-modal-open js-fullscreen-modal-open font-small col-12 mb-3">
