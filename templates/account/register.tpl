			...
			<div class="col-md-5">
				...
				{% if account_validation == 'pending' %}
					...
					<div class="mb-3 font-small">
						<p>{{ "¿Todavía no lo recibiste?" | translate }} <span class="js-resend-validation-link btn-link ml-1">{{ "Enviar link de nuevo" | translate }}</span></p>
					</div>
				...
				{% else %}
					...
					{% endembed %}
				{% endif %}
				<p class="mt-3 {% if account_validation == 'pending' %}text-left{% else %}text-center{% endif %}">{{ '¿Ya tenés una cuenta?' | translate }} {{ "Iniciá sesión" | translate | a_tag(store.customer_login_url, '', 'btn-link ml-1') }}</p>
			</div>
		</div>
	</div>
</section>
