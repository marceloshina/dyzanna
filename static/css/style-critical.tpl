...
[data-transition="scale-out-image"] img.lazyloaded { 
  transition: all .8s ease;
  opacity: 0;
  transform: scale(1.10);
}
[data-transition="scale-out-image"].is-inViewport img,
...
{% endraw %}
.nav-desktop-list {
  {% if settings.logo_position_desktop == 'center' and not settings.hamburger_desktop %}
    margin: 0 0 5px 0;
  {% else %}
    margin: 0;
  {% endif %}
...
@media (min-width: 768px) { 
  ...
  .label-top-right-md {
    {% endraw %}
    {% if settings.product_image_format == 'asymmetric' and settings.product_image_grid == '2' and (product.images_count > 1 or product.video_url) %}
      top: 40px;
    {% else %}
      top: 0;
    {% endif %}
  ...
  {% endraw %}
    ...
    {% endif %}
    {% if settings.logo_position_desktop == 'left' and not (settings.search_big_desktop and settings.hamburger_desktop) %}
      .desktop-search-cart-col {
        padding-bottom: 3px;
      }
    {% endif %}
  ...
  .filters-asymmetric {
    ...
    {% if settings.grid_columns_desktop == '4' %}
      right: 30px;
      bottom: -45px;
    {% elseif settings.grid_columns_desktop == '2' %}
  ...
  {% if settings.product_grid_asymmetric and settings.grid_columns_desktop == '3' %}
    .products-grid  .item-product:nth-child(3n - 1) {
      margin-top: 40px;
    }
  {% endif %}
  {% raw %}
